...
export default function InterestBox( {hobby, lightColor="#f70", darkColor="#f70"}: {hobby: {id: number, name: string, img: string, mylink: string }, lightColor?: string, darkColor?: string} ) {
  return (
    ...
    place-items-center-safe relative neon-box " style={{'--neon-dark-color': darkColor, '--neon-light-color': lightColor } as React.CSSProperties}> 
        <div className="min-h-0 h-1/5 w-full overflow-clip">
            <div className="py-2 sm:py-4 text-sm lg:text-lg text-balance text-white dark:text-orange-250">
              {hobby.name}
            </div>
        </div>
        <div className="min-h-0 h-4/5 w-full block place-items-center-safe p-2 cursor-pointer">
          <img className="max-h-full max-w-full inline h-auto w-auto object-fill md:object-fill md:h-fill md:w-fill
           py-2 " // max-h-8/10 max-w-8/10
          src={hobby.img}/>
        </div>
